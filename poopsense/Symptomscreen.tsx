...
interface SymptomScreenProps {
  ...
  onBack: () => void;
  onAnalyse: (symptoms: SymptomData) => void;
}

export const SymptomScreen: React.FC<SymptomScreenProps> = ({ dog, onBack, onAnalyse }) => {
        ...
        <div className="sym-pills">
          {NORMAL_SYMPTOMS.map((s) => (
            <button
              key={s}
              type="button"
              className={`sym-pill${selected.has(s) ? ' on' : ''}`}
              onClick={() => toggle(s)}
            >
              {s}
            </button>
        ...
        <div className="sym-pills">
          {BAD_SYMPTOMS.map((s) => (
            <button
              key={s}
              type="button"
              className={`sym-pill bad${selected.has(s) ? ' on' : ''}`}
              onClick={() => toggle(s)}
            >
              {s}
            </button>
