...
import { GalleryIcon, CameraIcon } from '../shared/Icons';
import { processImageFile, isSamsungBrowser } from '../../utils/imageUtils';
import type { ProcessedImage } from '../../utils/imageUtils';
import type { Dog, ScanEntry } from '../../types';
...
const DogStrip: React.FC<DogStripProps> = ({ dog, onEdit }) => (
...
interface TodayScansBarProps {
  scans: ScanEntry[];
  limit: number;
}

const TodayScansBar: React.FC<TodayScansBarProps> = ({ scans, limit }) => {
...
interface HomeVetCardProps {
  vetName: string;
  vetNum: string;
  lastEntry: ScanEntry | null;
  onShare: (name: string, num: string) => void;
  onSaveVet: (name: string, num: string) => void;
}

const HomeVetCard: React.FC<HomeVetCardProps> = ({ vetName, vetNum, lastEntry, onShare, onSaveVet }) => {
...
interface UploadScreenProps {
  ...
  lastEntry: ScanEntry | null;
  hasDogHistory: boolean;
  vetName: string;
  vetNum: string;
  onEditDog: () => void;
  onContinue: (image: ProcessedImage) => void;
  onNavHist: () => void;
  onNavProg: () => void;
...
export const UploadScreen: React.FC<UploadScreenProps> = ({
  ...
  const cameraRefSamsung = useRef<HTMLInputElement>(null);
  ...
  return (
    <div className="content">
      ...
      />
      <input
        ref={cameraRefSamsung} type="file" accept="image/*" capture="user" style={{ display: 'none' }}
        onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])}
      />
