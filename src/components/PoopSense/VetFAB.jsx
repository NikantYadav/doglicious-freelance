...
const VetFAB = ({ visible, currentEntry }) => {
  const { state } = useApp();
  ...
  const handleShare = () => {
    ...
    const summary = currentEntry.sum || currentEntry.clinicalSummary || 'PoopSense AI report available.';
    const dogName = dog?.name || 'my dog';
    const score = currentEntry.score || '?';

    const msg = encodeURIComponent(
      `[PoopSense AI] Hello Dr. ${state.vet?.name || 'Doctor'},\n\n` +
      `Dog: ${dogName}${dog?.breed ? ` (${dog.breed})` : ''} | ${dog ? `${dog.age || '?'}yr` : '?'} | ${dog?.wt || '?'}kg\n` +
      (dog ? `Parent: ${dog.parentName || ''}${dog.parentMobile ? ` (+91 ${dog.parentMobile})` : ''}\n` : '') +
