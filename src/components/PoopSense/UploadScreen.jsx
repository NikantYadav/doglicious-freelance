...
import CameraModal from "../VetRxScan/CameraModal";
  ...
  <div className="dog-strip">
    ...
    <div>
      ...
      <div className="ds-meta">
        {dog.breed} · {dog.age}yr{dog.wt ? ` · ${dog.wt}kg` : ""}
      </div>
    </div>
    <button className="ds-edit" onClick={onEdit} type="button">
      Edit
    </button>
  </div>
);
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span
            style={{ fontSize: 14, fontWeight: 800, color: "var(--brand)" }}
          >
            {count}/{limit}
          </span>
...
const HomeVetCard = ({
  ...
  const entryToShare =
    recentScans.find((s) => s.id === effectiveId) || lastEntry;
      ...
      <div className="home-vet-card">
        ...
        {hasVet && !showEdit ? (
          <div className="hvc-saved-vet">
            ...
            <span style={{ fontSize: 10, opacity: 0.7, color: "#fff" }}>
              +91 {vetNum}
            </span>
            <button
              className="hvc-change-btn"
...
export const UploadScreen = ({
  ...
  const handleCameraCapture = useCallback((photoData) => {
    setShowCamera(false);
    setImage({ url: photoData.url, b64: photoData.b64 });
    setHasImage(true);
  }, []);
  ...
  const openNativeCamera = () => {
    const ref = isSamsungBrowser()
      ? cameraRefSamsung.current
      : cameraRef.current;
    if (!ref) return;
    ref.value = "";
    ref.click();
    ...
    <>
      {showCamera && (
        <CameraModal
          ...
          onUseNativeCamera={isAndroidDevice() ? openNativeCamera : null}
      ...
      <div className="content">
        ...
        {!hasImage ? (
          ...
          >
            ...
            <p>
              Upload a clear, well-lit photo for instant AI health analysis.
              <br />
              Tap the button or anywhere in this box
            </p>
            <div
              style={{
                display: "flex",
        ...
        {todayScans.length > 0 && (
          <TodayScansBar scans={todayScans} limit={DAILY_LIMIT} />
        )}
