                ...
                <div style={{ maxWidth: "720px", margin: "0 auto" }} className="rv">
                    {FAQ_DATA.map((cat, i) => (
                        <React.Fragment key={i}>
                            <div style={{ fontSize: "12px", fontWeight: "700", textTransform: "uppercase", letterSpacing: ".08em", color: "var(--t3)", margin: i === 0 ? "0 0 20px" : "32px 0 20px" }}>
                                {cat.category}
                            </div>
                            <div className="faq-grid">
                                {cat.items.map(item => (
                                    <div className={`faq-item${openFaq === item.id ? ' open' : ''}`} key={item.id}>
